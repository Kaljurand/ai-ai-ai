...
* Allow users to:
  ...
  * Tabs are fixed to the top of the page. The tabs are:
    ...
    * **Log** – REST query log.
    * **Models** – browse OpenRouter models.
      The table lists each model's ID, name, description, architecture modality
      and pricing. Pricing is calculated by adding together the `prompt` and
      `completion` cost values returned by OpenRouter.
  ...
  * Clicking a textual table cell opens a pop-up with its Markdown-rendered content.
