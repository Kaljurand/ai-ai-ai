...
* Show remaining OpenRouter API credits next to its key in the Settings tab, refreshed whenever the tab is opened.
* Optional dark mode can be toggled from the Settings tab.
* System prompt fields for Text, Audio and ASR can be edited in the Settings tab. Their contents are prepended to all prompts before API calls.
