...
* Allow users to:
  ...
  * Audio files are listed in a sortable table showing ID, ISO timestamp, text ID, provider, length in seconds and audio format. Each row provides **Play**, **Download** and **Delete** actions.
  ...
  * Clicking any table row (except the action buttons and checkbox cells) opens a draggable pop-up showing all cell values. The header displays the row ID, timestamp and model or provider name together with a close button. The body includes the text, audio player and format field for audio rows. The footer repeats the row action buttons. Markdown, JSON and diff HTML render correctly and each field has a copy icon.
